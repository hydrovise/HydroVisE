...
function ini() {
  $(function () {
    ...
    if (config.timeSlider.hasOwnProperty("dynamicRange")) {
      is_dynamic = true;
      dynamicRange = new Function(
        config.timeSlider.dynamicRange.arguments,
        config.timeSlider.dynamicRange.body
      );
    }
    ...
    $("#slider").slider({
      ...
      slide: function (ev, ui) {
        ...
        let f = new Function(
          config.timeSlider.label.arguments,
          config.timeSlider.label.body
        );
  ...
  let attr_ctrl =
    config.hasOwnProperty("mapMarkers") &&
    config.mapMarkers.hasOwnProperty("markerAttrs");
  let prod_ctrl = config.controls.hasOwnProperty("prod");
  let ensemble_ctrl =
    Object.keys(config.traces).find((v) => {
  ...
  for (let i = 0; i < controlList.length; i++) {
    ...
    let lst_item =
      "<div class='baseMapTypeItem {4} {3}' data-value='{0}' onclick=\"{2}(this)\">{1}</div>";
  ...
  let lst_item =
    "<li id='li_{4}' class='ui-state-default'><span class='ui-icon ui-icon-arrowthick-2-n-s'></span>" +
  ...
  Object.keys(config.mapLayers).forEach((key) => {
    ...
    if (use_config[key].selected) {
      ...
      document.getElementById("check_lid_" + use_config[key].fn).classList =
        "checked";
  ...
  basemap = L.tileLayer(config.map.basemapList.default.url, {
