var rcCoef = {};
let rc_url = "examples/ex1_min/data/stage_discharge/rc_usgs.csv";
...
function evntQ2Stage(butt, hGrid) {
  ...
  let use_mod = Object.keys(c).filter((k) => {
    return c[k].button == butt;
  })[0];
  ...
  div_plot.data.forEach(
    // was_mod use_mod to make decisions on show hide ?
    (v) => {
      v.visible = !v.visible;
    }
  );
...
function Q2Stage(flow_arr) {
  ...
  translate_id
    ? (lid = translateID(systemState.comID))
    : (lid = systemState.comID);
  if (!rcCoef[lid]) {
    loadRatingCurve(lid);
  }
  if (rcCoef[lid] == undefined) return [];
  return flow_arr.map((val) =>
    val == "" ? "" : polynomFunction(val, rcCoef[lid])
  );
}
