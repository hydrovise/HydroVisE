...
function vColor(_var, _val) {
    ...
    // console.log(_val, rv, rvs, rvsn)
    return arr_color[_var].color_table[rvsn]
}


function generateColorBar() {
    var colors = arr_color['kge'].color_table;
    var step = arr_color['kge'].step;
    var vmin = arr_color['kge'].min;
    var rev = arr_color['kge'].reverse;
    var show_min = arr_color['kge'].show_min;
    var zero = 0;
    var rnd = arr_color['kge'].round;
    // console.log(rnd)
    bins = Array.apply(
...
//             console.log(element)
    ...
    //$("#colorbar").css('background-color', 'black');
...
// bins =  Array.apply(
//     null, Array(colors.length)
// ).map(function (_, i) {return Math.round((i*step+vmin)*rnd)/rnd;});
//
// var zip = bins.map(
//     function(e, i) {
//         return [
...
// zip.forEach(function(element) {
//
//         str_div_bars += '<div class="bar-row" style="height: {2}px; background-color:{1}"><div class="bar bar-label">{0}</div></div>'.format(element[0], element[1], bar_height);
//     }
// );
// //$("#colorbar").css('background-color', 'black');
// $("#colorbar").replaceWith(str_div_bars + '</div>');
