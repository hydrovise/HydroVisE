...
function pathInterpret($path){
    ...
    $_args = [];
    //echo "\n\n", count($_arr), "\n\n";
    if (count($_arr) != 4) return False;
    
    $_args += xy2x_y($_arr[3]);
    $_args += array("prd"=>$_arr[1]);
    $_args += array("yr"=>$_arr[2]);
...
$conn = pg_connect("host=s-iihr51.iihr.uiowa.edu dbname=$dbname user=rgoska password=A!neta1972 port=5435") or die('connection failed');
