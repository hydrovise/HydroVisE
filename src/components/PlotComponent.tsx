import React, { useEffect, useRef } from 'react';
import Plot from 'react-plotly.js';
import { useApp } from '../context/AppContext';
import { usePlot } from '../context/PlotContext';
import Papa from 'papaparse';

interface PlotComponentProps {
  isVisible: boolean;
  onToggle: () => void;
}

export default function PlotComponent({ isVisible }: PlotComponentProps) {
  const plotRef = useRef<any>(null);
  const { state, dispatch } = useApp();
  const { plotOperations } = usePlot();

  const [plotData, setPlotData] = React.useState<any[]>([]);
  const [plotLayout, setPlotLayout] = React.useState<any>({
    title: 'Time Series Plot',
    xaxis: { title: 'Time' },
    yaxis: { title: 'Value' },
    showlegend: true,
    autosize: true,
    margin: { l: 50, r: 50, t: 50, b: 40 }
  });
  const [loading, setLoading] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);

  // Register plot operations with context
  useEffect(() => {
    plotOperations.current = {
      addTraces: async (lifespan = 'semi-permanent') => {
        await handleAddTraces(lifespan);
      },
      clearTraces: (lifespan = 'semi-permanent') => {
        handleClearTraces(lifespan);
      },
      updateSliderState: (value: number) => {
        dispatch({
          type: 'UPDATE_SYSTEM_STATE',
          payload: { sliderState: value }
        });
      }
    };
  }, [dispatch]);

  useEffect(() => {
    if (!state.config) return;

    // Initialize plot layout from config
    if (state.config.plotlyLayout) {
      setPlotLayout((prevLayout: any) => ({
        ...prevLayout,
        ...state.config!.plotlyLayout,
        autosize: true
      }));
    }
  }, [state.config]);

  const formatArray = (template: string, values: any[]): string => {
    let result = template;
    values.forEach((value, index) => {
      result = result.replace(`{${index}}`, String(value));
    });
    return result;
  };

  const generatePlotTitle = (comID: string | number) => {
    if (!state.config?.mapMarkers?.plotTitle) {
      return `Time Series - comID: ${comID}`;
    }

    const titleConfig = state.config.mapMarkers.plotTitle.template;
    // For now, use a simple title format
    // In full implementation, would look up marker features for dynamic title
    return formatArray(titleConfig.format, [comID]);
  };

  const loadTraceData = async (year: number | string, comID: string | number) => {
    if (!state.config) return;

    setLoading(true);
    setError(null);

    const traces = state.config.traces;
    const newData: any[] = [];

    for (const [traceKey, traceConfig] of Object.entries(traces)) {
      try {
        // Build URL from template
        const vars = traceConfig.template.var;
        const pathFormat = traceConfig.template.path_format;

        let url = pathFormat;
        vars.forEach((varName, index) => {
          const value = varName === 'yr' ? year :
            varName === 'comID' ? comID : '';
          url = url.replace(`{${index}}`, String(value));
        });

        console.log(`Loading trace ${traceKey} from:`, url);

        // Fetch and parse CSV data using PapaParse for better CSV handling
        const response = await fetch(url);
        if (!response.ok) {
          console.warn(`HTTP ${response.status} for trace ${traceKey}`);
          continue;
        }

        const csvText = await response.text();

        // Parse CSV with PapaParse
        const parseResult = Papa.parse(csvText, {
          header: true,
          skipEmptyLines: true,
          dynamicTyping: true
        });

        if (parseResult.errors.length > 0) {
          console.warn(`CSV parse errors for ${traceKey}:`, parseResult.errors);
        }

        const data = parseResult.data as any[];
        if (!data.length) continue;

        const x: any[] = [];
        const y: any[] = [];

        data.forEach(row => {
          const xValue = row[traceConfig.x_name];
          const yValue = row[traceConfig.y_name];

          if (xValue !== undefined && yValue !== undefined && yValue !== null) {
            x.push(xValue);
            y.push(Number(yValue));
          }
        });

        if (x.length === 0) continue;

        newData.push({
          x,
          y,
          ...traceConfig.style,
          name: traceConfig.style.name || traceKey
        });
      } catch (error) {
        console.warn(`Failed to load trace ${traceKey}:`, error);
      }
    }

    if (newData.length === 0) {
      setError('No data could be loaded for the selected location and year.');
    } else {
      setError(null);
    }

    setPlotData(newData);
    setLoading(false);

    // Update plot title
    const title = generatePlotTitle(comID);
    setPlotLayout((prevLayout: any) => ({
      ...prevLayout,
      title: title
    }));
  };

  // Handle adding traces (for timeSlider functionality)
  const handleAddTraces = async (lifespan: string = 'semi-permanent') => {
    if (!state.config?.traces || !state.systemState.comID) return;

    const traces = state.config.traces;
    const addingTraces: any[] = [];

    for (const [traceKey, traceConfig] of Object.entries(traces)) {
      // Only process dynamic traces
      if (!traceConfig.dynamic) continue;

      // Check if ensemble trace matches current product
      if (traceConfig.ensemble && state.systemState.prod !== traceConfig.prod) {
        continue;
      }

      try {
        // Generate path using current system state (including sliderState)
        const vars = traceConfig.template.var;
        const pathFormat = traceConfig.template.path_format;

        let url = pathFormat;
        vars.forEach((varName, index) => {
          let value = '';
          switch (varName) {
            case 'yr':
              value = String(state.systemState.yr);
              break;
            case 'comID':
              value = String(state.systemState.comID);
              break;
            case 'sliderState':
              value = String(state.systemState.sliderState);
              break;
            default:
              value = String((state.systemState as any)[varName] || '');
          }
          url = url.replace(`{${index}}`, value);
        });

        console.log(`Adding dynamic trace ${traceKey} from:`, url);

        const response = await fetch(url);
        if (!response.ok) continue;

        const csvText = await response.text();
        const parseResult = Papa.parse(csvText, {
          header: true,
          skipEmptyLines: true,
          dynamicTyping: true
        });

        const data = parseResult.data as any[];
        if (!data.length) continue;

        const x: any[] = [];
        const y: any[] = [];

        data.forEach(row => {
          const xValue = row[traceConfig.x_name];
          const yValue = row[traceConfig.y_name];

          if (xValue !== undefined && yValue !== undefined && yValue !== null) {
            x.push(xValue);
            y.push(Number(yValue));
          }
        });

        if (x.length === 0) continue;

        addingTraces.push({
          x,
          y,
          ...traceConfig.style,
          name: traceConfig.style.name || traceKey,
          lifespan: lifespan,
          showlegend: true
        });

      } catch (error) {
        console.warn(`Failed to add trace ${traceKey}:`, error);
      }
    }

    if (addingTraces.length > 0) {
      setPlotData(prevData => [...prevData, ...addingTraces]);
    }
  };

  // Handle clearing traces (for timeSlider functionality)
  const handleClearTraces = (lifespan: string = 'semi-permanent') => {
    setPlotData(prevData =>
      prevData.filter(trace => {
        if (!trace.lifespan) return true; // Keep traces without lifespan

        if (lifespan === 'semi-permanent') {
          return false; // Remove all traces with lifespan
        } else {
          return trace.lifespan !== lifespan; // Remove only matching lifespan
        }
      })
    );
  };

  // Load data when comID changes
  useEffect(() => {
    if (state.systemState.comID && state.systemState.yr) {
      loadTraceData(state.systemState.yr, state.systemState.comID);
    }
  }, [state.systemState.comID, state.systemState.yr, state.config]);

  if (!isVisible) {
    return null;
  }

  return (
    <>
      {error && (
        <div style={{
          padding: '20px',
          backgroundColor: '#fee',
          border: '1px solid #fcc',
          borderRadius: '4px',
          margin: '10px',
          color: '#c00'
        }}>
          Error loading data: {error}
        </div>
      )}

      {!loading && !error && plotData.length > 0 && (
        <Plot
          ref={plotRef}
          data={plotData}
          layout={{
            ...plotLayout,
            autosize: true,
            height: 350, // Fixed height to fit within div_plot
            margin: { l: 50, r: 50, t: 50, b: 40 }
          }}
          style={{ width: '100%', height: '350px' }}
          useResizeHandler={true}
          config={{
            displayModeBar: true,
            displaylogo: false,
            modeBarButtonsToRemove: ['pan2d', 'lasso2d'],
            modeBarButtonsToAdd: [{
              name: 'Download plot as PNG',
              title: 'Download plot as PNG',
              icon: {
                'width': 1792,
                'path': 'M512 1376v288q0 40-28 68t-68 28h-320q-40 0-68-28t-28-68v-288q0-40 28-68t68-28h320q40 0 68 28t28 68zM1792 1376v288q0 40-28 68t-68 28h-320q-40 0-68-28t-28-68v-288q0-40 28-68t68-28h320q40 0 68 28t28 68zM512 640v288q0 40-28 68t-68 28h-320q-40 0-68-28t-28-68v-288q0-40 28-68t68-28h320q40 0 68 28t28 68zM1792 640v288q0 40-28 68t-68 28h-320q-40 0-68-28t-28-68v-288q0-40 28-68t68-28h320q40 0 68 28t28 68zM512 96v192q0 40-28 68t-68 28h-320q-40 0-68-28t-28-68v-192q0-40 28-68t68-28h320q40 0 68 28t28 68zM1792 96v192q0 40-28 68t-68 28h-320q-40 0-68-28t-28-68v-192q0-40 28-68t68-28h320q40 0 68 28t28 68z',
                'height': 1792
              },
              click: (gd: any) => {
                // Use Plotly's built-in download functionality
                const Plotly = (window as any).Plotly;
                if (Plotly && Plotly.downloadImage) {
                  Plotly.downloadImage(gd);
                }
              }
            }]
          }}
        />
      )}

      {loading && (
        <div style={{
          display: 'flex',
          justifyContent: 'center',
          alignItems: 'center',
          height: '300px',
          fontSize: '18px',
          color: '#666'
        }}>
          Loading time series data...
        </div>
      )}

      {!loading && plotData.length === 0 && !error && (
        <div style={{
          display: 'flex',
          justifyContent: 'center',
          alignItems: 'center',
          height: '300px',
          fontSize: '18px',
          color: '#666'
        }}>
          Click on a map feature to load time series data
        </div>
      )}
    </>
  );
}